...
int get_student(int fd, int id, student_t *s) {
    ...
    if (bytes_read == -1) {
        return ERR_DB_FILE;
    } else if (id < MIN_STD_ID || id > MAX_STD_ID || bytes_read < (ssize_t)sizeof(student_t) || s->id == DELETED_STUDENT_ID) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    if (bytes_read == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    } else if (bytes_read < (ssize_t)sizeof(student_t)) {
 ...
 */
 int count_db_records(int fd) {
    ...
    printf(record_count == 0 ? M_DB_EMPTY : M_DB_RECORD_CNT, record_count);
    return record_count;
}
...
int print_db(int fd) {
    ...
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t)) {
        if (student.id != DELETED_STUDENT_ID) {
            ...
            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, (float)student.gpa / 100.0);
        }
    }
...
int compress_db(int fd) {
    ...
    int temp_fd = open(TMP_DB_FILE, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    if (temp_fd == -1) {
        printf(M_ERR_DB_OPEN);
        close(fd);
        return ERR_DB_FILE;
